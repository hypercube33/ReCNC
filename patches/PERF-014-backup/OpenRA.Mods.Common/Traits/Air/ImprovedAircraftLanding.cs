...
namespace OpenRA.Mods.Common.Traits
{
	...
	public sealed class ImprovedAircraftLanding : IAircraftLanding
	{
		...
		/// </summary>
		readonly Dictionary<Actor, Queue<LandingRequest>> landingQueues = new();
		...
		int CalculatePadScore(Actor aircraft, Actor pad, int priority)
		{
			...
			if (queue != null && queue.Any(r => r.Aircraft == aircraft))
		...
		public IDisposable ReserveLandingZone(Actor aircraft, Actor landingActor)
		{
			...
			{
				queue = new Queue<LandingRequest>();
				landingQueues[landingActor] = queue;
			}

			if (!queue.Any(r => r.Aircraft == aircraft))
			{
				if (queue.Count < MaxQueuePerPad)
				{
					...
					if (priority > 1 && queue.Count > 0)
					{
						var items = queue.ToList();
						items.Add(request);
						items = items.OrderByDescending(r => r.Priority).ThenBy(r => r.RequestTick).ToList();
						queue.Clear();
		...
		void ReleaseLandingZoneInternal(Actor aircraft, Actor landingActor)
		{
			...
			if (landingQueues.TryGetValue(landingActor, out var queue))
			{
				var items = queue.Where(r => r.Aircraft != aircraft).ToList();
				queue.Clear();
				foreach (var item in items)
					queue.Enqueue(item);
			}
		...
		public bool IsLandingZoneAvailableFor(Actor aircraft, Actor landingActor)
		{
			...
			if (landingQueues.TryGetValue(landingActor, out var queue))
			{
				if (queue.Count >= MaxQueuePerPad && !queue.Any(r => r.Aircraft == aircraft))
					return false;

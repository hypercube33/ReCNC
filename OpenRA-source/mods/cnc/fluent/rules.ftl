...
## ai.yaml
# BEGIN ReCnC UI-001 — rename TD AI bots to standardized difficulty names
# Mapping (see mods/cnc/rules/ai.yaml):
#   easy        <- watson  (balanced production, predictable squad sizes)
#   medium      <- cabal   (larger squads, artillery-heavy, no helipads)
#   hard        <- hal9001 (smaller focused squads, highest building limits)
#   specialist  -> NEW    (turtle/defensive build, doubled defense fractions)
bot-easy =
    .name = Easy
...
bot-specialist =
    .name = Specialist (Turtle)
# END ReCnC UI-001

## ReCnC Algorithm Options
dropdown-pathfinding =
    .label = Pathfinding Algorithm
    .description = Algorithm used for unit pathfinding

dropdown-aircraft =
    .label = Aircraft Landing
    .description = Algorithm for aircraft landing pad selection

dropdown-docking =
    .label = Docking Algorithm
    .description = Algorithm for harvester/repair dock selection

dropdown-algorithm-openra =
    .label = OpenRA (Default)

dropdown-algorithm-classic =
    .label = Classic C&C

dropdown-algorithm-improved =
    .label = Improved

...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class PathFinder : IPathFinder, IWorldLoaded
	{
		...
		/// <summary>
		/// Strategy manager for selecting between OpenRA and Classic C&amp;C pathfinding algorithms.
		/// </summary>
		PathfindingStrategyManager strategyManager;
		...
		/// <summary>
		/// Gets the current pathfinding algorithm being used.
		/// </summary>
		public PathfindingAlgorithm CurrentAlgorithm => strategyManager?.CurrentAlgorithm ?? PathfindingAlgorithm.OpenRA;
		...
		public void WorldLoaded(World w, WorldRenderer wr)
		{
			...
			// Initialize the strategy manager for algorithm selection
			strategyManager = new PathfindingStrategyManager(world);
			foreach (var kvp in hierarchicalPathFindersBlockedByNoneByLocomotor)
				strategyManager.RegisterOpenRAStrategy(kvp.Key, kvp.Value);
		}
			...
			Func<CPos, int> customCost, Actor ignoreActor, bool inReverse, bool laneBias)
		{
			...
			if (sources.Count == 1)
			{
				...
				// Use the selected pathfinding strategy (OpenRA HPA* or Classic C&C)
				var hpf = GetHierarchicalPathFinder(locomotor, check, ignoreActor);
				var strategy = strategyManager.GetStrategy(locomotor, hpf);
			...
			// Use the selected pathfinding strategy for multi-source search
			var hpfMulti = GetHierarchicalPathFinder(locomotor, check, ignoreActor);
			var strategyMulti = strategyManager.GetStrategy(locomotor, hpfMulti);
			return strategyMulti.FindPath(
		...
		{
			var hpf = hierarchicalPathFindersBlockedByNoneByLocomotor[locomotor];
			var strategy = strategyManager.GetStrategy(locomotor, hpf);
			return strategy.PathExists(source, target);
		}
		...
		{
			var hpf = hierarchicalPathFindersBlockedByImmovableByLocomotor[locomotor];
			var strategy = strategyManager.GetStrategy(locomotor, hpf);
			return strategy.PathExists(source, target);
		}

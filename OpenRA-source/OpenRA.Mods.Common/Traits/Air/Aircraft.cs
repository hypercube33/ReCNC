...
namespace OpenRA.Mods.Common.Traits
{
		...
		IActorPreviewInitInfo, IEditorActorOptions
	{
		...
		[Desc("Horizontal tolerance from helipad/resupplier center while landed; Resupply uses this instead of WDist.Zero so repair and rearm can run when not exactly centered.")]
		public readonly WDist GroundResupplyProximity = WDist.FromCells(3);
		...
		IActorPreviewInitModifier, IDeathActorInitModifier, IIssueDeployOrder, IIssueOrder, IResolveOrder, IOrderVoice
	{
		...
		Actor preferredResupplier;

		public Actor PreferredResupplier
		{
			get
			{
				if (preferredResupplier != null && (!preferredResupplier.IsInWorld || preferredResupplier.IsDead))
					preferredResupplier = null;

				return preferredResupplier;
			}
		}

...
namespace OpenRA.Mods.Common.Activities
{
	public class ReturnToBase : Activity
	{
		...
		public ReturnToBase(Actor self, Actor dest = null, bool alwaysLand = false)
		{
			...
			this.dest = dest ?? aircraft.PreferredResupplier;
		...
		public static Actor ChooseResupplier(Actor self, bool unreservedOnly)
		{
			...
			var sticky = aircraft.PreferredResupplier;

			if (sticky != null && !sticky.IsDead && sticky.IsInWorld && sticky.Owner == self.Owner
				&& rearmInfo.RearmActors.Contains(sticky.Info.Name))
			{
				if (!unreservedOnly || Reservable.IsAvailableFor(sticky, self))
					return sticky;
		...
		public override bool Tick(Actor self)
		{
			...
			{
				var sticky = aircraft.PreferredResupplier;
				var nearestResupplier = sticky != null && !sticky.IsDead ? sticky : ChooseResupplier(self, false);
			...
			if (ShouldLandAtBuilding(self, dest))
			{
				...
				QueueChild(new Resupply(self, dest, aircraft.Info.GroundResupplyProximity, alwaysLand));
				return true;
			}
